...
- **Never verify** — `record_implementation` lands an item at `implemented`, NOT `done`. The autopilot is unattended, so there is no human to confirm the work; it records the implementation and stops. Reaching `done` is always a present human's decision (verify_action_item is not even available here). **But DO record what you observed.** Unattended does not mean silent: if an acceptance criterion can be settled by running or querying something, check it and record it with `record_criterion_verdicts`. Recording an honest observation is NOT self-verification — the server decides whether the criteria add up, not you. Evidence must name what you RAN or QUERIED and what CAME BACK; reading code is inference and is rejected. Anything only a person can judge (taste, "does this look right") gets `classify_criterion` with a reason, never a tick — and never `unknown`. Keep the two apart: `unknown` means "nobody has checked this YET" and stays retryable by anything better equipped, while `classify_criterion` means no tool ever could. Marking a line as needing a person merely because YOU could not check it, unattended, dumps mechanical work on a human — that is the failure these two states exist to separate.
...
1. Read the repo for repo facts: `README`, `CONTRIBUTING`, architectural notes, and the directory you are about to modify — layout, build, how the pieces fit together.
...
When you discover something worth persisting — a deviation from the item's stated approach ("the item said X, we did Y because Z"), a non-obvious constraint, an architectural finding — record it with `record_memory` (`decision`/`convention`/`architecture`/`risk`/`insight`). ALWAYS `search_memories` first and `supersede_memory` the closest match instead of duplicating. `search_memories` returns a CARD (title, one-line summary, id) — `get_memory` the closest match and read it in full before superseding it, because a card is enough to choose WHICH memory you mean and not enough to justify replacing it.  Do NOT record transient details or anything obvious from the code. Your writes land **unconfirmed** and capped at `in_discussion` — you propose; the human ratifies. This is DevSpec's **shared** team memory — not your own local memory (OpenCode's `AGENTS.md` / instructions file): durable, shared project knowledge goes to DevSpec `record_memory`, while personal or machine-specific notes stay in your local memory — that boundary is what keeps DevSpec from going stale.
