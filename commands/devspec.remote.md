...
2. **Resolve the project, then register.** Discover the origin URL with a read-only file tool or a conservatively read-only Git shell command. If inspecting files, find the nearest `.git` entry at or above the working directory; follow `gitdir:` and `commondir` pointers when present to reach the common config. From `[remote "origin"]`, use the `url` value as `git_remote`. If no origin resolves unambiguously, omit `git_remote` rather than guessing.
   ...
   **Folder pin — a project whose code does not exist yet.** Look for `.devspec/project.json` with a silent existence check (glob or list the path). Do not `Read` a file that is not there — hosts paint that miss as a failed tool. Search the working directory first, then each parent up to the git repository root, never at or above your home directory (`~/.devspec` is machine state, not project config); nearest wins. **Then the repository's main working tree, if the chain found nothing:** the pin is normally untracked, so a linked worktree holds no copy of it, and the implementation contract asks for isolated worktrees. `git rev-parse --path-format=absolute --git-common-dir` is one identity for a repository and all of its worktrees; its parent is the main working tree when it ends in `.git`. Jurisdiction is a property of the repository rather than of the directory you happen to be in — see `commit_provenance_contract.project_association` in the served contract. Only `Read` the pin when the existence check found it. It holds `{ "project_id": "<uuid>" }`. A missing pin plus a working git origin is the normal quiet path, not an error. **If nothing resolved — no pin and no remote — offer to create one** once the user names the project: write `{ "project_id": "<uuid>" }` at the repository root, or at the working directory when there is no repo. Do not offer a pin when origin already identifies the project. **Never write it silently**, put nothing but the project id in it (no path, hostname, user or timestamp — that is what makes it safe to commit), and if a pin already names a DIFFERENT project, say which one before replacing it.
...
- A durable decision, convention, architecture choice or risk → a memory. `search_memories` first; the result is a card, so `get_memory` the closest match and read it in full. Then record, supersede, link or ask the owner as the comparison warrants — the closest match is not automatically the one to replace. Types: `decision`, `convention`, `architecture`, `risk`, `insight`.
...
- **Read each write's `outcome` and report what it says.** `committed` = recorded now. `queued_for_review` = **not in effect**: relay the `proposal_id` and the reason the server gave, and say a maintainer has to accept it on the project's Review page, under Proposed changes, before it applies. Who may commit which kind of write, and why a call was queued, is the served contract's decision — `devspec://product/implementation-contract` → `authority_contract` — not this command's; do not predict the outcome, read it.
- **Open questions about records live on Review.** Flags, contradictions and changes waiting for a maintainer are on the project's Review queue (`get_review_queue`). Before ruling on a flag, `preview_conflict_resolution` shows what the ruling would do; `manage_knowledge_links` records that two records belong together without settling anything; `get_action_item_history` shows what changed on an item. Who may rule is the served contract's decision, not this file's.
