...
- **Permission waits** (items bb633917 / 464635b3): a permission wait is **not** model progress, but it is also not a terminal stall. `permission.asked` records the request ID and preserves the active attempt, command ownership, reply correlation, and work trail for as long as OpenCode remains resumable. A matching `permission.replied` clears only that request and restarts the ordinary progress window; stale replies cannot clear newer asks. The room receives customer-safe guidance to approve or deny the prompt in the OpenCode terminal. If the event is missed, message-part detection (`messageHasPendingPermissionAsk`) fails safe by retaining the wait until OpenCode settles the session.
- **Prompt-based and allow-all configurations**: with OpenCode's default prompt permissions, the remote turn stays visibly active until somebody approves or denies the local terminal prompt. DevSpec cannot answer that local-only prompt remotely. With allow-all permissions, no wait event is emitted and the turn continues normally. The plugin still declares a bonded `permission.ask` auto-allow hook for OpenCode versions that invoke it, but OpenCode 1.18.21 does not call that hook; configured OpenCode permission rules remain authoritative.
