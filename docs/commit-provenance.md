...
| **Project association** | Plugin `directory` plus `.devspec/project.json`; `/devspec.remote` already *offers* to write a pin | Jurisdiction is a well-formed pin walked cwd→parents, never at `$HOME`. The plugin never writes the pin. No pin means no deny and no nudge | Ingestion never depends on a pin |
