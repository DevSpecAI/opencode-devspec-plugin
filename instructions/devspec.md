...
4. **Record what you did.** Once the work is pushed, call `record_implementation` with the commit SHA, affected files, and testing notes. Never mark an item `done` yourself, and never call `verify_action_item` unless a human explicitly asks — signing off is theirs. But DO record what you observed: any acceptance criterion you can settle by running or querying something goes in `record_criterion_verdicts`, with evidence naming what you RAN or QUERIED and what CAME BACK. That is not signing off — the server computes promotion from the criteria, and an item can reach `done` on evidence alone. Reading code is inference and is rejected. Anything only a person can judge (taste, "does this look right") gets `classify_criterion` with a reason, never a tick — and never `unknown`. Keep the two apart: `unknown` means "nobody has checked this YET" and stays retryable by anything better equipped, while `classify_criterion` means no tool ever could. The canonical lifecycle is: staged → claimed → implementing → implemented → verified → done.
