...
export const ACTIVE_PLAN_PROJECTION_VERSION = 1 as const
...
export const ACTIVE_SESSION_PLAN_AUTHORITY_NOTE =
  'Advisory read-awareness only. Presence does not authorize execution or mutation; manage_plan still requires a capability-authenticated caller identity, explicit plan_id for cross-plan work, and expected_revision.' as const
...
export interface ActiveSessionPlan {
  ...
  orphaned: boolean
  progress: { terminal: number; total: number; completed: number; skipped: number }
...
export interface ActiveSessionPlansProjection {
  ...
  inventory: { returned: number; total_known: number; truncated: false }
...
function planIdentity(v: unknown, at: string): asserts v is ActiveSessionPlanAgentIdentity {
  ...
  if ((v.agent_name as string).length > ACTIVE_SESSION_PLAN_MAX_IDENTITY_CHARS ||
  ...
  if (v.kind === 'dev') {
    if (v.connection_id !== null) throw new Error(`${at}.connection_id must be null for Dev`)
  } else {
    uuid(v.connection_id, `${at}.connection_id`)
  }
}

function planStep(v: unknown, at: string): asserts v is ActiveSessionPlanStep {
  if (!record(v)) throw new Error(`${at} must be an object`)
  ...
  uuid(v.id, `${at}.id`)
  if (!Number.isSafeInteger(v.position)) throw new Error(`${at}.position is invalid`)
  str(v.title, `${at}.title`)
  if ((v.title as string).length > ACTIVE_SESSION_PLAN_MAX_TITLE_CHARS) throw new Error(`${at}.title is too large`)
  ...
  if (v.status === 'failed') {
    if (v.failure_reason !== undefined) {
      ...
      if ((v.failure_reason as string).length > ACTIVE_SESSION_PLAN_MAX_FAILURE_REASON_CHARS) throw new Error(`${at}.failure_reason is too large`)
    }
    if (typeof v.retryable !== 'boolean') throw new Error(`${at}.retryable is required for a failed step`)
  }
}

function activePlans(v: unknown, at: string): asserts v is ActiveSessionPlansProjection {
  exact(v, ['version', 'advisory', 'authority_note', 'inventory', 'plans'], at)
  if (v.version !== ACTIVE_PLAN_PROJECTION_VERSION || v.advisory !== true ||
      v.authority_note !== ACTIVE_SESSION_PLAN_AUTHORITY_NOTE) throw new Error(`${at} version/authority is unsupported`)
  exact(v.inventory, ['returned', 'total_known', 'truncated'], `${at}.inventory`)
  if (!Array.isArray(v.plans) || v.plans.length < 1 || v.plans.length > ACTIVE_SESSION_PLAN_MAX_PLANS) throw new Error(`${at}.plans count is invalid`)
  if (v.inventory.returned !== v.plans.length || v.inventory.total_known !== v.plans.length || v.inventory.truncated !== false) throw new Error(`${at}.inventory must describe a complete projection`)
  let totalText = 0
  v.plans.forEach((rawPlan, index) => {
    ...
    if ((rawPlan.title as string).length > ACTIVE_SESSION_PLAN_MAX_TITLE_CHARS) throw new Error(`${planAt}.title is too large`)
    if (!Number.isSafeInteger(rawPlan.revision) || (rawPlan.revision as number) < 1 || rawPlan.status !== 'active' || !validOffsetDateTime(rawPlan.created_at)) throw new Error(`${planAt} revision/status/time is invalid`)
    ...
    if ((rawPlan.owner.display_name as string).length > ACTIVE_SESSION_PLAN_MAX_IDENTITY_CHARS) throw new Error(`${planAt}.owner.display_name is too large`)
    if (typeof rawPlan.orphaned !== 'boolean') throw new Error(`${planAt}.orphaned is invalid`)
    exact(rawPlan.progress, ['terminal', 'total', 'completed', 'skipped'], `${planAt}.progress`)
    for (const key of ['terminal', 'total', 'completed', 'skipped'] as const) {
      if (!Number.isSafeInteger(rawPlan.progress[key]) || (rawPlan.progress[key] as number) < 0) throw new Error(`${planAt}.progress.${key} is invalid`)
    }
    if (!Array.isArray(rawPlan.steps) || rawPlan.steps.length > ACTIVE_SESSION_PLAN_MAX_STEPS) throw new Error(`${planAt}.steps count is invalid`)
    ...
    totalText += (rawPlan.title as string).length + rawPlan.origin.agent_name.length + (rawPlan.origin.codename?.length ?? 0) +
      rawPlan.steward.agent_name.length + (rawPlan.steward.codename?.length ?? 0) + (rawPlan.owner.display_name as string).length +
      rawPlan.steps.reduce((sum, step) => sum + step.title.length + (step.failure_reason?.length ?? 0), 0)
  })
  if (totalText > ACTIVE_SESSION_PLAN_MAX_TOTAL_TEXT_CHARS) throw new Error(`${at} text is too large`)
}

function bounded(v: unknown, at: string, policyVersion: string): asserts v is CanonicalWindow {
  exact(v, ['policy_version', 'returned', 'total_known', 'source_window', 'truncated', 'has_more', 'next_cursor', 'fetch_id', 'omission_reason'], at)
  if (v.policy_version !== policyVersion || !Number.isSafeInteger(v.returned) || (v.returned as number) < 0) throw new Error(`${at} version/count is invalid`)
...
export function parseCanonicalIngress(input: unknown, expectedConnectionId: string): CanonicalIngressResult {
  try {
    if (!record(input)) throw new Error('ingress must be an object')
    ...
    if (!enhanced && !scoped) throw new Error('ingress contract/policy pair is unsupported')
    ...
    if (input.kind !== 'devspec.remote_ingress' || input.schema_version !== 1) throw new Error('ingress discriminator is unsupported')
    if (enhanced && hasActivePlans) activePlans(input.active_session_plans, 'ingress.active_session_plans')
...
const PLAN_STEP_MARK: Record<ActiveSessionPlanStep['status'], string> = {
  pending: '[ ]',
  in_progress: '[>]',
  completed: '[x]',
  failed: '[!]',
...
function planAgentLabel(identity: ActiveSessionPlanAgentIdentity): string {
 ...
 */
export function serializeActiveSessionPlans(
  projection: ActiveSessionPlansProjection | undefined,
  recipient: { connectionId: string; ownerUserId?: string | null },
): string {
  ...
  for (const plan of projection.plans) {
    ...
    lines.push(
      '',
      `${access} ${JSON.stringify(plan.title)} | plan_id=${plan.id} | revision=${plan.revision} | ` +
    ...
    for (const step of plan.steps) {
      lines.push(
        `  ${PLAN_STEP_MARK[step.status]} ${step.position}. ${step.title} | step_id=${step.id} | status=${step.status}` +
          (step.status === 'failed'
    ...
    if (own) {
      ...
      lines.push(
        `  Lifecycle: continue this plan or end it explicitly; complete only if the outcome and steps are achieved, otherwise abandon with a specific reason. Closing the plan does not end the session. Never silently restart/drop it. Every mutation uses plan_id=${plan.id} and expected_revision=${plan.revision}.`,
        allTerminal
