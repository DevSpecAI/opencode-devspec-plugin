...
import { shouldDeferInjectDuringConnect, } from './remote-format.js';
 ...
 */
export async function markOwnerGone(directory) {
    const auth = resolveDevspecAuth(directory);
    const state = readState();
    ...
    try {
        ...
        logPoll(`markOwnerGone: heartbeat sent (status:offline, busy:false, end_reason:owner_gone) for ${connectionId} ` +
            `(was busy=${wasBusy})`);
    }
    catch (err) {
 ...
 */
export async function checkPermissionWaitTimeout(directory) {
    const state = readState();
    if (!state?.busy || !state.permissionAskedPending || !state.permissionAskedAt)
        return;
    ...
    if (elapsed >= PERMISSION_WAIT_AUTO_CLEAR_MS) {
        logPoll(`permission wait timeout: ${elapsedMinutes}m since first ask — auto-clearing busy so ` +
        ...
        if (auth.ok && state.sessionId) {
            await postSessionNotice(auth, state, `OpenCode was waiting on a permission prompt for over ${Math.round(PERMISSION_WAIT_AUTO_CLEAR_MS / 60_000)} minutes ` +
    ...
    if (Date.now() - lastNotice < PERMISSION_WAIT_NOTICE_REPEAT_MS)
        return;
    if (!auth.ok || !state.sessionId)
        return;
    const remainingMinutes = Math.max(1, Math.round((PERMISSION_WAIT_AUTO_CLEAR_MS - elapsed) / 60_000));
    await postSessionNotice(auth, state, `OpenCode is still waiting for permission (${elapsedMinutes}m so far). ` +
    ...
    logPoll(`permission wait still pending at ${elapsedMinutes}m — follow-up advisory posted; auto-clear in ${remainingMinutes}m`);
}
...
const hostAcceptedQuestionReplies = new Set();
export function parseNeedsInputReply(content) {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            return null;
        const row = parsed;
        if (typeof row.requestId !== 'string' || !row.requestId.trim() || !Array.isArray(row.answers))
            return null;
        const answers = row.answers.map((answer) => {
            if (!Array.isArray(answer))
                throw new Error('invalid answer row');
            ...
            if (values.length === 0)
                throw new Error('empty answer row');
            return values;
        });
        if (answers.length === 0)
            return null;
...
export async function replyPendingQuestion(input) {
    ...
    if (hostAcceptedQuestionReplies.has(requestId)) {
        ...
        catch { /* retry the local write on the next poll */ }
        return true;
    }
    try {
        if (!questionClient)
...
function recordConnectionEventInBond(isRegister, args, hookOutput, opencodeSessionId) {
    ...
    if (connectionOwner && connectionOwner !== opencodeSessionId) {
        logPoll(`attach observation: opencodeSession=${opencodeSessionId} sharing connection ` +
...
function persistedAutomationDispatchIds(state) {
    ...
    return {
        ids: [...new Set(legacy.filter((id) => typeof id === 'string'))].slice(-50),
...
export async function pollAndDeliver(client, directory, sessionId, opts = {}) {
    ...
    if (!state.pendingQuestion?.requestId && markedReplyIds.length > 0) {
        for (const id of markedReplyIds)
            deliveredIds.add(id);
        patchState({ deliveredMessageIds: [...deliveredIds].slice(-50) });
        commands.splice(0, commands.length, ...commands.filter((command) => !markedReplyIds.includes(command.message_id)));
    ...
    if (state.pendingQuestion?.requestId && commands.length > 0) {
        const matching = parsedReplies.find(({ reply }) => reply?.requestId === state.pendingQuestion?.requestId);
        ...
        if (replied) {
            ...
            const remainingCommands = commands.filter((command) => command.message_id !== answerCommandId && !staleIds.includes(command.message_id));
            const stages = [
                acceptanceStage('needs_input_delivered_ids', answerAcceptanceKey, () => {
                    ...
                    if (!patchState({
                        ...
                        currentTurnMessageIds: Array.from(new Set([...(acceptedState.currentTurnMessageIds ?? []), answerCommandId])).slice(-50),
                    })) {
            ...
            if (remainingCommands.length === 0) {
                stages.push(acceptanceStage('needs_input_conversation_cursor', answerAcceptanceKey, () => {
                    if (!commitConversationCursor())
                        throw new Error('conversation cursor was not persisted');
                }), acceptanceStage('needs_input_advisory_carry', answerAcceptanceKey, () => { pump.carry.take(); }), acceptanceStage('needs_input_deferred_clear', answerAcceptanceKey, () => {
...
export async function deliverInjectedTurn(input) {
    const { client, directory, sessionId, auth, text, fileParts, model, thinking, answerCorrelation, onAccepted, onRejected, shouldCleanupRejectedTurn, } = input;
    let state = readState();
...
export function clearAbandonedInjectCursor(baseline) {
    logPoll(`clearing abandoned inject cursor — baseline ${baseline} not in current OpenCode session`);
    clearInjectTurnState({ unclaim: true });
...
function trailGuardKey(sessionId) {
    return sessionId;
}
...
function isPostSessionMessageToolName(toolName) {
...
export function claimAgentAnswerPost(callId) {
    ...
    const currentReservation = state.answerPostInFlight && state.answerPostProcessId === ANSWER_POST_PROCESS_ID;
    ...
    if (!claimed)
        throw new Error('DevSpec: could not reserve answer delivery');
    return claimed;
}
/** Release the prior turn's success latch when OpenCode starts a new user turn. */
export function resetAnswerPostLatchForUserTurn() {
    const state = readState();
    if (!state)
        return;
...
export async function resolveCurrentAssistantModel(client, sessionId, callId) {
    try {
        const response = await withTimeout(client.session.messages({ path: { id: sessionId } }), OPENCODE_SESSION_API_TIMEOUT_MS, 'session.messages(answer-model)');
        ...
        const firing = assistants.findLast((message) => Array.isArray(message?.parts) && message.parts.some((part) => part?.callID === callId || part?.callId === callId || part?.call?.id === callId));
        if (!firing) {
        ...
        logPoll(`answer post: current model unavailable (${resolved.missingReason ?? 'absent'}) ` +
...
export function settleAgentPostResult(toolName, result, callId) {
    if (!isPostSessionMessageToolName(toolName))
        return false;
 ...
 */
export async function handleSessionIdle(directory) {
    const state = readState();
    if (!state)
        return;
