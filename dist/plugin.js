...
function isRegisterConnectionTool(tool) {
...
function isPostSessionMessageTool(tool) {
...
function isPermissionResolvedEvent(type) {
...
function isQuestionAskedEvent(type) {
    return type === 'question.asked' || type === 'question.v2.asked';
}
function isQuestionResolvedEvent(type) {
...
export const DevSpecPlugin = async ({ client, directory }) => {
                ...
                for (const sessionId of sessions) {
                    ...
                    try {
                        ...
                        if (outcome.stop) {
                            ...
                            logPoll(`pump: bond ended for opencodeSession=${sessionId} — ${outcome.reason ?? 'connection ended'} ` +
                                `(remaining=${listOpenCodeBondSessions().length})`);
                            continue;
                        }
                        if (outcome.delayMs < minDelay)
                            minDelay = outcome.delayMs;
                    }
                    catch (err) {
                        ...
                        if (IDLE_RECHECK_MS < minDelay)
                            minDelay = IDLE_RECHECK_MS;
                    }
                }
    ...
    return {
        ...
        'permission.ask': async (input, output) => {
            ...
            if (!askingSession || !isBondedOpenCodeSession(askingSession))
                return;
            if (!shouldAutoAllowRemoteControlPermission())
                return;
            output.status = 'allow';
            ...
            const patternPreview = Array.isArray(patterns)
                ...
                : typeof patterns === 'string'
                    ? patterns
                    : '';
            logPoll(`permission.ask auto-allow (remote-control bond active) type=${kind}` +
        ...
        event: async ({ event }) => {
            ...
            const eventInfo = props?.info && typeof props.info === 'object'
                ? props.info
                : undefined;
            ...
            try {
                ...
                if (propsSummary.length > 500)
                    propsSummary = `${propsSummary.slice(0, 500)}…`;
            }
            catch {
            ...
            const inBond = (fn) => runWithBondAsync(sessionId, fn);
        ...
        'tool.execute.before': async (input, output) => {
            ...
            if (isPostSessionMessageTool(input.tool)) {
                const bonded = typeof input.sessionID === 'string' && isBondedOpenCodeSession(input.sessionID);
                if (bonded) {
                    logPoll(`post_session_message refused for opencodeSession=${input.sessionID} — ` +
                        `plugin owns egress while a bond is active`);
                    throw new Error('DevSpec: this OpenCode session is connected to DevSpec, and the plugin posts your ' +
            ...
            try {
                if (!isRegisterConnectionTool(input.tool))
                    return;
                ...
                logPoll(`register_connection: local_id supplied for opencodeSession=${input.sessionID}` +
