...
const NAME_SET = new Set(OPENCODE_CONTROL_SLASH_NAMES);
/**
 ...
 */
export function parseOpencodeControlSlash(raw) {
 ...
 */
export function resolveOwnerControlSlash(commands) {
    ...
    const structured = only.content && typeof only.content === 'object'
        ? only.content
        : null;
    const content = typeof only.content === 'string'
        ? only.content
...
export function controlSlashSuccessMessage(cmd) {
    switch (cmd.kind) {
        case 'compact':
